...
\pagestyle{ueg_mat_heading}
{
	...
	%
	% -> Ambiente Enumerate contendo a 
	%	 estrutura dos capítulos do TC
	%
	\begin{enumerate}[align=left,leftmargin=*,label=\textbf{Capítulo \arabic*}:]
		...
		\estruturaApendice {Regra de Leibniz}
	\end{enumerate}
	...
	% inserir o conteúdo do arquivo contendo a
	% tabela para simplificar a exibição\edição
	% deste documentos
	\input{tabela_cronograma.tex}
	% Conferir questão do tamanho da fonte na tabela
	...
	% há um bug em que o texto aqui passa pra
	% baixo da margem, não resolvido ainda.
	% controlar de forma manual.
}
\clearpage
...
\textual
{
	% inserir o conteúdo do arquivo
	% para simplificar a exibição/edição
	% deste arquivo.
	\input{texto.tex}
