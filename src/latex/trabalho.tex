...
% ---
\titulo{\textbf{Uma Introdução ao Cálculo Variacional e ao Método de Rayleigh-Ritz com Aplicações em Python.}}
...
% ---
\makeindex
\makeindex[name=remissivo,title=\'Indice Remissivo,options=-s indexstyle,columns=2]
...
\begin{itemize}
	...
	\item \index[remissivo]{SciPy}\textit{SciPy library} contém módulos para matemática, ciências e engenharia. As funções trazidas pela \textit{SciPy} permitem o trabalho com álgebra linear, cálculo, interpolação, manipulação de arquivos, otimização, processamento de sinais, entre outros \cite{SciPy}.
...
$$
	...
	\text{,}
$$
onde as funções $\phi_i(x)$ são chamadas de \index[remissivo]{Função de Forma}funções de forma e devem ser linearmente independentes. As funções $\phi_i(x)$ devem ser, também, diferenciáveis até a ordem $m$, onde $m$ é a ordem da maior derivada do funcional. Os coeficientes $a_i \in \mathbb{R}$ são chamados de parâmetros de deslocamento e $v(x)$ é chamada de \index[remissivo]{Função Aproximadora}função aproximadora.
...
\citeonline{MRR_Deflex} apresenta o uso de funções triangulares como funções de forma, visando a discretização da função $y(x)$. Considere o funcional
\begin{equation}
...
Tanto a derivação quanto a integração podem ser feitas manualmente ou utilizando o pacote \textit{SymPy} em Python, como feito no Código \ref{py_code_solve_mrr_ex_livro_diff}.
...
\begin{lstlisting}[style=Python, xleftmargin=2em, numbers=left, firstnumber=31, caption={Algoritmo para integração numérica usando a regra dos trapézios}, captionpos=t, label=py_code_solve_mrr_tri_integrate]
...
\begin{lstlisting}[style=Python, xleftmargin=2em, numbers=left, firstnumber=43, caption={Funções utilizadas para criação das matrizes do sistema linear}, captionpos=t, label=py_code_solve_mrr_tri_m_b]
def p(x):
    return e**x
...
\begin{lstlisting}[style=Python, xleftmargin=2em, numbers=left, firstnumber=68, caption={Definição e resolução do sistema linear}, captionpos=t, label=py_code_solve_mrr_tri_solvesys]
M = zeros(n, n+1)
for i in range(0, n):
    M[i, n] = b(i)
    M[i, i] = m(i, i)
...
Utilizando a solução do Código \ref{py_code_solve_mrr_tri_solvesys} e a função exata $y(x)=(x-1)(e^{-x}-1)$, é possível criar funções, numéricas, representando a função exata e a função aproximada, plotando-as no mesmo gráfico, como feito no Código \ref{py_code_solve_mrr_tri_plot}.

\begin{lstlisting}[style=Python, xleftmargin=2em, numbers=left, firstnumber=78, caption={Plotagem das funções exata e aproximada}, captionpos=t, label=py_code_solve_mrr_tri_plot]
