...
A construção do sistema $M\cdot A=B$ e sua resolução, pelo \index[remissivo]{Método de Gauss-Jordan}Método de Gauss-Jordan, será implementado na linguagem de programação Python na Seção \ref{sec:python_triangular} do Capítulo \ref{cap:python}.

%O sistema construído $M\cdot A=B$ será resolvido utilizando o \index[remissivo]{Método de Gauss-Jordan}Método de Gauss-Jordan, com implementação na linguagem de programação Python na Seção \ref{sec:python_triangular} do Capítulo \ref{cap:python}.

%O sistema construído $M\cdot A=B$ será resolvido, utilizando linguagem de programação Python na Seção \ref{sec:python_triangular} do Capítulo \ref{cap:python}.
...
\label{apend:python}
{
	...
	{\color{red}Escrever sobre a instalação.}
	
	\section{Executando Programas}
	
	{\color{red}Explicar sobre a linguagem Python.}
	
	\section{Principais Operações}
	
	Para facilitar o entendimento dos códigos ao longo do trabalho, é apresentada uma lista com algumas das principais operações na linguagem \textit{Python} na Tabela \ref{table:principais_operacoes}.
