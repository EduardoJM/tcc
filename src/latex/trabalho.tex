...
Reescrevendo a integral (\ref{eqn:cap_calc_var:int_funcional}) utilizando as funções aproximadoras definidas, tem-se
...
Sabe-se que $\frac{\partial f}{\partial y'}\eta \Big |_{x_1}^{x_2}=0$, devido ao fato de que $\eta(x_1)=\eta(x_2)=0$, portanto, \eqref{eqn:cap_calc_var:integration_parts} pode ser reescrita como
$$
I'(0)=
	\int_{x_1}^{x_2} \frac{\partial f}{\partial y}\eta dx
	-
...
Em \eqref{eqn:cap_calc_var:func_n_derivadas_pos_separar} deve-se integrar o segundo membro por partes uma vez, o terceiro membro por partes duas vezes, o quarto membro três vezes e assim por diante, até que o $n$-ésimo membro seja integrado $n-1$ vezes, também por partes. No fim das integrações, a equação de Euler-Lagrange será obtida fazendo $I'(0)=0$, isto é, aplicando a condição de estacionariedade e, por último, utilizando o Lema \ref{lema:cap_calcvar_lema_1}.
...
O segundo membro pode ser integrado, por partes, com $u=\frac{\partial f}{\partial y'}$ e $dv=\eta'dx$, donde 
$du=\frac{d}{dx} \left (
	\frac{\partial f}{\partial y'}
...
O terceiro membro deve ser integrado duas vezes por partes. Na primeira integração, seja $u=\frac{\partial f}{\partial y''}$ e $dv=\eta''dx$, onde 
$du=\frac{d}{dx} \left (
	\frac{\partial f}{\partial y''}
...
Usando os resultados \eqref{eqn:cap_calc_var:limit_diff_x1} e \eqref{eqn:cap_calc_var:limit_diff_x2}, $\frac{\partial f}{\partial y''}\eta ' \Big |_{x_1}^{x_2} = 0$, portanto
$$
	\int_{x_1}^{x_2}
		\frac{\partial f}{\partial y''}
		\eta'' dx
...
Para funcionais que dependem de derivadas superiores até a ordem $n$, o processo para obtenção da equação é o mesmo e a equação geral é

{\color{red}Escrever a formula da equação geral.}

\chapter{Método de Rayleigh-Ritz}

O objetivo do Cálculo Variacional é o de encontrar uma função $y(x)$ que, dentre as funções admissíveis torna o funcional estacionário. Uma das formas para se encontrar a função $y(x)$ é o de resolver a equação de Euler-Lagrange associada ao problema. É evidente que a resolução de equações diferenciais é uma tarefa trabalhosa e, em muitos casos, até infazível. Visando esquivar desses problemas surgem métodos que, atualmente, são trabalhados computacionalmente, dentre os quais está o Método de Rayleigh-Ritz.
