...
O operador $\delta$ é denominado operador variacional. \citeonline{haroldo_aplic_espacial} elucida que o operador diferencial $\delta$ e o operador diferencial $d$ representam mudanças diferenciais infinitesimais de uma função $y$, porém, $dy$ se refere a uma mudança de $y(x)$ causada pela variação $dx$, ou seja, {\color{red}$dy=y(x+dx)-y(x)$} enquanto que $\delta y$ é uma variação de $y(x)$ na forma de uma nova função $\delta y=\varepsilon \eta(x)$.
