...
$$
	...
	\text{,}
$$
onde as funções $\phi_i(x)$ são chamadas de funções de forma e devem ser linearmente independente. As funções $\phi_i(x)$ devem ser, também, diferenciáveis até a ordem $m$, onde $m$ é a ordem da maior derivada do funcional. Os coeficientes $a_i \in \mathbb{R}$ são chamados de parâmetros de deslocamento e $v(x)$ é chamada de função aproximadora.
