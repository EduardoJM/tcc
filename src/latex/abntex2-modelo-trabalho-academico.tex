...
\end{equation}
onde $\eta (x)$ é uma função diferenciável arbitrária para a qual $\eta (x_1)=\eta (x_2) = 0$. O número $\epsilon$ é o parâmetro da família. É possível escrever, também, a derivada de $Y$ como
...
\begin{figure}[!h]
	...
	\centering
	\includegraphics[width=0.7\textwidth, angle=0]{figuras/cap_calcvar/figura_001.pdf}
	\label{fig:test_01}
...
Note que a função procurada $y(x)$ é o membro da família $Y(x)$ quando $\epsilon = 0$. Ou seja, se $\epsilon = 0$ pode-se substituir $Y$ e $Y'$ por $y$ e $y'$, respectivamente. Deste modo, a integral (\ref{eqn:int_funcional_approx}) fornece os mesmos extremos que a (\ref{eqn:int_funcional}) quando $\epsilon=0$.
...
Derivando \eqref{eqn:cap_calcvar_eq_approx} em função de $\epsilon$, tem-se $\frac{\partial Y}{\partial \epsilon}=\frac{\partial y}{\partial \epsilon}+\frac{\partial}{\partial \epsilon}(\epsilon \eta)$, de onde conclui-se que $\frac{\partial Y}{\partial \epsilon}=\eta$, pois $y$ e $\eta$ independem de $\epsilon$. O mesmo acontece com \eqref{eqn:cap_calcvar_eq_approx_diff}, donde verifica-se que $\frac{\partial Y'}{\partial \epsilon}=\eta'$. Deste modo, de \eqref{eqn:cap_calcvar_chain_rule}, obtêm-se a integral
...
Para calcular $I'(0)$, tem-se que $\epsilon=0$ e, então, podemos trocar $Y$ e $Y'$ por $y$ e $y'$, respectivamente, obtendo
$$
...
\label{apend:regra_de_leibniz}
{
	...
	\begin{teorema}
		...
		\begin{proof}
		Suponha que o teorema não seja válido, então existiriam $\varepsilon > 0$ e sequências de pontos $x_k\in X$, $\alpha_k \in K$ tais que $|x_k-x_0|<\frac{1}{k}$ e $|f(x_k,\alpha_k)-f(x_0,\alpha_k)|\geqslant \varepsilon$. Passando a uma subsequência, se necessário e adimitindo que $lim \alpha_k=\alpha \in K$. Como $lim x_k=x_0$, devido a continuidade de $f$ tem-se $\varepsilon \leqslant lim |f(x_k,\alpha_k)-f(x_0,\alpha _k)|=|f(x_0,\alpha)-f(x_0,\alpha)|$, uma contradição, pois da hipótese $\varepsilon >0$.
		\end{proof}
	\end{teorema}
	
	\begin{teorema}[Derivação sob o sinal de integral]
		...
		\begin{proof}
			...
			Usando o \textbf{\color{red}Teorema (do Valor Médio?)} pode-se escrever \eqref{eqn:anexo_leibniz_r1} como
			\begin{equation}
				...
				\right ] dt\text{,}
			\end{equation}
			onde $\theta \in [0,1]$. Como $\frac{\partial f}{\partial x_i}$ é contínua e $[a,b]$ é compacto, pelo Teorema \ref{teorema:func_uniformemente}, para todo $\varepsilon > 0$, existe um $\delta > 0$, de modo que
			\begin{equation}
			...
			\right | dt\text{,}
			$$
			e, utilizando \eqref{eqn:anexo_leibniz_ineq_func_unif}, pode-se escrever a inequação como
			$$
			\left |
				\int_a^b \left [
					\frac{\partial f}{\partial x_i}(x+\theta s e_i, t)-\frac{\partial f}{\partial x_i}(x,t)
			...
			\right |
			<
			\int_a^b \frac{\varepsilon}{b-a} dt \text{,}
			$$
			ou, desenvolvendo a integral da direita, como
			\begin{equation}
			...
			De \eqref{eqn:anexo_leibniz_voltas} e \eqref{eqn:anexo_leibniz_ineq_limit}, tem-se que
			$$
				|s|<\delta \Longrightarrow
				\left |
					\frac{\varphi(x+se_i)-\varphi(x)}{s}
