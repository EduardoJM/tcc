...
%	\end{frame}
	...
	\begin{frame}
		...
		$$
			...
			\text{,}
		$$
		associado ao Problema de Valor de Contorno que modela a deflexão de uma viga apoiada sobre seus extremos.
	\end{frame}
	...
	\begin{frame}
		...
		Substituindo a função aproximada
		$$
	...
	\begin{frame}
		...
		Seja a função aproximada $v_3(x)$,
		$$
...
print("Maxima deflexao: ", maxdef)
	...
	\begin{frame}
		\begin{figure}
			...
			\includegraphics[scale=1.6]{../figuras/code/code_solve_mrr_exemplo_livro_presentation.png}
			\fonteElaboradaPeloAutor
			%\\{\footnotesize Fonte: Elaborada pelo autor, 2019.}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		$$
			b_j=\int_{0}^{1} 2f(x)\phi_j(x)dx
		$$
		pode ser resolvido numericamente utilizando a linguagem Python.
	\end{frame}
	...
	\begin{frame}
		...
		Para comparação do método, será utilizado um exemplo onde $p(x)=e^x$, $q(x)=e^x$ e $f(x)=x+(2-x)e^x$. A função exata para esse problema é conhecida, $y(x)=(x-1)(e^{-x}-1)$.
	\end{frame}
	...
	\makesubtitleframe{FIM!\\\vspace{20pt}OBRIGADO!}
